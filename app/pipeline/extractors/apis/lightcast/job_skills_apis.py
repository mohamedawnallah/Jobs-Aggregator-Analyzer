...
import aiohttp
from pipeline.extractors.apis.lightcast.access_token_api import LightCastAccessTokenGenerator
...
class LightCastJobSkillsAPIs:
    ...
    def __init__(self, etl_configs: str, lightcast_credentials: LightCastCredentials):
        ...
        self.lightcast_access_token_generator = LightCastAccessTokenGenerator()

    async def get_latest_job_skills(self) -> List[dict]:
        """Get the latest skills"""
        self.access_token = await self.lightcast_access_token_generator.extract(self.access_token_api, self.lightcast_grant_type, self.lightcast_client_id, self.lightcast_secret, self.lightcast_scope)
        ...
        while True:
            ...
            if response.status == 401:
                self.access_token = await self.lightcast_access_token_generator.extract(self.access_token_api, self.lightcast_credentials, self.lightcast_client_id, self.lightcast_secret, self.lightcast_scope)
                continue
            break
        try:
